...
- **New:** 
	- Il processo **viene creato** ma non è ancora pronto per l'esecuzione
- **Ready:** 
	- Il processo è **pronto per l'esecuzione** ma non sta utilizzando le risorse del pc (cpu, memoria)
- **Suspend/Ready:**
	- Il processo è sospeso ed in stato di *ready*.
- **Run:**
	- Il processo viene eseguito e utilizza le risorse del pc (cpu, memoria).
- **Wait/Block:**
	- Il processo è **in attesa di un evento esterno** per continuare l'esecuzione, ad esempio un input dell'utente o un segnale di sistema
- **Suspend/Wait:**
	- Il processo viene sospeso temporaneamente ed in stato di *wait*.
- **Termination:**
	- Il processo è stato **completato** o **interrotto** e vengono restituiti i risultati e **deallocate le risorse**
...
- **Tempo di risposta:**
	- Il **tempo trascorso tra la richiesta di un processo e la sua prima risposta**. Questa metrica è importante per le applicazioni interattive, in cui un tempo di risposta rapido è essenziale per un'esperienza utente soddisfacente.
- **Tempo di attesa:**
...
- **Fairness:**
	- La capacità del sistema di **distribuire equamente le risorse** tra processi
...
**First in First Out** è un algoritmo molto semplice che come dice il nome il primo job che arriva sarà anche il primo ad essere eseguito, questo algoritmo andrebbe bene se ogni processo avesse lo stesso tempo di esecuzione ma in un contesto reale questa cosa è impossibile.

Questo approccio è sconsigliato perché altamente inefficiente come vedremo con il prossimo capitolo.

## Shortest Job First

In questo caso per evitare che il un processo più veloce rallenti la coda, si decide di far eseguire prima i job più corti in modo tale che il tempo di esecuzione si riduca.
...
- i processi più corti potrebbero arrivare dopo e quindi verrebbe eseguito prima i processi più lungi.
- Potrebbero accodarsi tantissimi processi corti impedendo l'esecuzione di processi più lungi (**starvation**).
...
**Multi-Level Feedback Queue** è un algoritmo che gestisce lo scheduling analizzando le operazioni passate dei processi e in base a questo assegnare ad ognuno di essi una priorità di esecuzione.

I processi vengono inseriti in code a diversi livelli a seconda della loro priorità, la priorità viene scelta in base a:
- Il **tempo di utilizzo** della CPU di un processo.
- Che **tipo di operazione** esegue il processo (CPU o I/O).
Se per esempio un processo fa I/O ha una priorità maggiore in quanto occupa meno tempo l'esecuzione.

Solitamente comunque i processi con **priorità più alta** hanno un **tempo di esecuzione breve** mentre i processi con **priorità più bassa** hanno un **tempo di esecuzione maggiore**

Anche qui come il round robin un job potrebbe rimanere in priorità bassa e non venire mai eseguito.
...
**Completely Fair Scheduler** è uno degli scheduler utilizzati da Linux, esso si basa sulla teoria del **fair share** ovvero **distribuire in modo equo le risorse** del sistema tra tutti i processi in esecuzione.

Il CFS non alloca le risorse in base ad un livello di priorità ma piuttosto controlla:
- La quantità di risorse utilizzate dal processo 
- Il tempo trascorso dal processo in attesa di risorse (**virtual runtime**)

La coda di schedulazione del CFS è un **albero binario red-black** ordinato per virtual runtime, vedi [[05. Alberi]] dove per trovare il minimo impiego $\log(n)$ tempo. 
...
Cos'è la **starvation**:: Quando tanti  processi con un tempo di esecuzione minore impediscono l'esecuzione di processi con un tempo di esecuzione maggiore.
<!--SR:!2023-01-17,1,223-->

Nel **MLFQ** in base a cosa è data la priorità
?
- Al **tipo di operazione** svolta dalla cpu.
- **tempo di esecuzione** del processo.
<!--SR:!2023-01-17,1,216-->

Perché si usa un **albero binario red-black** per la coda dei processi nel **CFS**:: Per avere un tempo $\log(n)$ nella **ricerca del minimo**.
<!--SR:!2023-01-17,1,230-->