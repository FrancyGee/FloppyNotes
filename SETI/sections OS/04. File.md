...
Su Linux ogni cosa è un file e ogni file ha:
- un **file descriptor**
...
>La compilazione è il processo di **traduzione** del codice sorgente **da linguaggio di programmazione a linguaggio macchina**.
...
Il preprocessore è la **prima fase** della compilazione esso si occupa di:
- Elaborare le **Macro**
...
Al fine di ciò crea (per quanto riguarda il C/C++) un file con estensione `.i` o `.ii` o `.s`.
...
Il **linking statico** è il **processo di combinazione dei file di codice durante la compilazione**. In questo caso, **le librerie vengono incluse direttamente nel file eseguibile**, il che significa che **il file eseguibile risultante è più grande** ma non dipende da alcun file esterno per funzionare.

Il **linking dinamico** è il **processo di combinazione dei file di codice in fase di esecuzione**. In questo caso, **le librerie non vengono incluse direttamente nel file eseguibile, ma vengono caricate solo quando il programma le richiede**. Ciò significa che **il file eseguibile risultante è più piccolo** ma dipende da file esterni per funzionare.

## Eseguibile

Un eseguibile è il prodotto finito della compilazione, esso contiene:
- **Codice Macchina**
- **Dati**
- **Metadati**
	- architettura della CPU
	- **entry-point** (l'entry-point di un eseguibile non è il main ma da esso possiamo risalire all'indirizzo di memoria del `main`).
...
In un sistema Unix/Linux un file è un eseguibile, una libreria o un file oggetto di linker, si trova nel formato **ELF (Executable and Linkable Format)**
Questo tipo di file è:
- **Estendibile**
- **Flessibile**
- **Multipiattaforma**
In un file ELF noi troviamo
...
Quali sono i tre livelli di permessi per i file
?
- **User (u)**
- **Group User (g)**
- **Others (o)**
...
Quali permessi gestisce il livello **User** per i file:: i permessi dell'utente
<!--SR:!2023-01-17,1,220-->

Quali permessi gestisce il livello **Group** per i file:: i permessi del gruppo dell'utente
<!--SR:!2023-01-17,1,225-->

Quali permessi gestisce il livello **User** per i file:: i permessi degli altri utenti
<!--SR:!2023-01-17,1,228-->
